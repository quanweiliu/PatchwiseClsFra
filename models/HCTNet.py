...
class HCTNet(nn.Module):
    def __init__(
        ...
        self.conv2d_features = nn.Sequential(
            nn.Conv2d(in_channels=8*18, out_channels=64, kernel_size=(3, 3)),
            nn.BatchNorm2d(64),
            nn.ReLU(),
        )
    ...
    def forward(self, x1, x2, mask = None):
        ...
        # print("x1", x1.shape)  # 64, 8, 18, 9, 9

        x1 = rearrange(x1, 'b c h w y ->b (c h) w y')
        # print("x2", x1.shape)  # 64, 144, 9, 9
...
if __name__ == '__main__':
    ...
    model = HCTNet(in_channels=4)
    model.eval()
    # print(model)
    input1 = torch.randn(64, 20, 11, 11)
    input2 = torch.randn(64, 4, 11, 11)
