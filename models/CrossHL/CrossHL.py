...
class CrossHL_Transformer(nn.Module):
    ...
    def forward(self, x1, x2):
        x1 = x1.reshape(x1.shape[0], 1, -1, self.patchsize, self.patchsize)
        x2 = x2.reshape(x1.shape[0], 1, -1, self.patchsize*self.patchsize)
        # x1 = x1.unsqueeze(1)
        # print("x1", x1.shape)
        # print("x2", x2.shape)
...
if __name__ == '__main__':
    model = CrossHL_Transformer(16, 20, 1, 6, 11)
    x1 = torch.randn(64, 20, 11, 11)
    x2 = torch.randn(64, 1, 11, 11)
    out = model(x1, x2)
    print(out.shape)
    # print(out)