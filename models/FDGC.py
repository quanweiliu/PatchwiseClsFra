...
class FDGC(nn.Module):
    def __init__(self, input_channels, num_classes, patch_size, \
                 factors=8, drop_prob=0.1, block_size=3):
        ...
        # third branch
        self.fc_sam = nn.Conv2d(64, self.num_node, (1,1), bias=False)  # in , out, kernel
    ...
    def forward(self, x):
        ...
        x7 = self.forward_sam(x2)   # 8, 64, 15, 15  -> 8, 64, 32 转化为节点特征
        # print("x7", x7.shape)
        x7 = self.gcn(x7) + x7
        # print(x7.shape)             # 8, 64, 32
...
if __name__ == "__main__":
    input = torch.rand(8,32,19,19)
    model = net = FDGC(input_channels=32, num_classes=16, patch_size=19)
    output = model(input)
    print(output.shape)