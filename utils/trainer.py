...
def train(epoch, net, net_head, criterion, train_loader, test_loader, train_optimizer, args):
    ...
    for pos_1, pos_2, label in train_bar:
        ...
        label = label - 1
        pos_1, label = pos_1.cuda(non_blocking=True), label.cuda(non_blocking=True)
    ...
    if epoch % args.log_interval == 0:
        ...
        for t_pos_1, t_pos_2, t_label in test_loader:
            with torch.no_grad():
                t_label = t_label - 1
                t_pos_1, t_label = t_pos_1.cuda(non_blocking=True), t_label.cuda(non_blocking=True)
...
def train_SMIMO(epoch, net, criterion, train_loader, test_loader, optimizer, args):
    ...
    if epoch % args.log_interval == 0:
        ...
        for t_pos_1, t_pos_2, t_label in test_loader:
            with torch.no_grad():
                t_label = t_label - 1
                t_pos_1 = t_pos_1.cuda(non_blocking=True)
                t_pos_2 = t_pos_2.cuda(non_blocking=True)
...
def train_MMISO(epoch, net, criterion, train_loader, test_loader, optimizer, args):
    ...
    return round(loss.cpu().item(), 4), round(train_accuracy, 4), round(test_accuracy, 4), round(train_time, 2)
